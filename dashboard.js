...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.loadCalendar = function (containerId = 'calendarEvents') {
    const el = document.getElementById(containerId);
    if (!el) return;
    el.innerHTML = `<div class="loading"><i class="fa-solid fa-spinner fa-spin"></i> Loading calendar events...</div>`;
      ...
      .then(data => {
        if (data.connected && data.events) {
          if (data.events.length === 0) {
            el.innerHTML = `<div class="empty-state">No upcoming events scheduled.</div>`;
            return;
          }

          el.innerHTML = data.events.slice(0, 5).map(ev => {
            ...
            return `
              <div class="event-item">
                <div class="event-date">${month}<br/>${day}</div>
                <div class="event-main">
                  <div class="item-title">${ev.summary || 'Untitled Event'}</div>
                  <div class="item-meta">${timeStr} ${ev.location ? '• ' + ev.location : ''}</div>
                </div>
                ${ev.htmlLink ? `<a href="${ev.htmlLink}" target="_blank" class="open-link">View →</a>` : ''}
              </div>
            `;
          }).join('');
        } else {
          el.innerHTML = `<div class="error-state">${data.detail || 'Connect Google Calendar to see events.'}</div>`;
        }
      })
      .catch(() => {
        el.innerHTML = `<div class="error-state">Error loading Google Calendar events.</div>`;
      });
  };
